...
| -T2    | nmap 192.168.1.1 -T2           | Polite (2) slows down the scan to use less bandwidth and use fewer target machine resources |
...
| Switch | Example | Description |
|--------|---------|-------------|
| --host-timeout | --host-timeout 1s; 4m; 2h | Give up on a target after this long |
...
| --max-retries | --max-retries 3 | Specify the maximum number of port scan probe retransmissions |
| --min-rate | --min-rate 100 | Send packets no slower than <number> per second |
| --max-rate | --max-rate 100 | Send packets no faster than <number> per second |
---
...
| --script     | nmap 192.168.1.1 --script default                | Scan with default NSE scripts. Considered useful for discovery and safe |
...
---
## Nmap Evasion and Output Options
